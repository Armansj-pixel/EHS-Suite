...
export default function NearMissListPage() {
  ...
  const [items, setItems] = useState<NearMissDoc[]>([]);
  ...
  async function loadFirst() {
    ...
    try {
      ...
      const arr: NearMissDoc[] = snap.docs.map((d) => ({ id: d.id, ...(d.data() as any) }));
      setItems(arr);
      setLast(snap.docs[snap.docs.length - 1] || null);
      setHasMore(snap.docs.length === PAGE_SIZE);
    } catch (e: any) {
      console.error(e);
      setErr(e?.message || "Gagal memuat Near Miss.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function closeNearMiss(id: string) {
    try {
      ...
      console.error(e);
      alert(e?.message || "Gagal menutup Near Miss (cek izin / rules).");
    }
  }

  function canClose(item: NearMissDoc) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold">Near Miss</h1>
        <div className="flex gap-2">
          <Link href="/nearmiss/new" className="px-3 py-2 text-sm bg-gray-900 text-white rounded hover:bg-gray-800">
            + Near Miss
          </Link>
      ...
      {!loading && items.length === 0 ? (
        <div className="text-sm text-gray-500">Belum ada laporan Near Miss.</div>
      ) : (
              ...
              {items.map((it) => (
                <tr key={it.id} className="border-b last:border-none">
                  ...
                  <td className="py-2 px-3">
                    <Link href={`/nearmiss/${it.id}`} className="hover:underline">
                      {it.title || "-"}
                    </Link>
                  </td>
