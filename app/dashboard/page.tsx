...
export const dynamic = "force-dynamic"; // amanin prerender
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border rounded-xl px-4 py-3 flex items-center justify-between relative">
        ...
        <div className="relative">
          {ready ? (
            ...
            >
              <img src={avatarUrl} alt="Profile" className="w-9 h-9 rounded-full border" />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Link href="/hazards/new" className="bg-white border rounded-xl p-4 hover:bg-gray-50 transition">
          <div className="text-sm font-medium">+ Hazard</div>
          <div className="text-xs text-gray-500">Buat laporan hazard</div>
        </Link>
        <Link href="/nearmiss/new" className="bg-white border rounded-xl p-4 hover:bg-gray-50 transition">
          <div className="text-sm font-medium">+ Near Miss</div>
          <div className="text-xs text-gray-500">Catat kejadian hampir celaka</div>
        </Link>
        <Link href="/inspections" className="bg-white border rounded-xl p-4 hover:bg-gray-50 transition">
          <div className="text-sm font-medium">Inspeksi</div>
          <div className="text-xs text-gray-500">Checklist & rekap inspeksi</div>
        </Link>
        <Link href="/hazards" className="bg-white border rounded-xl p-4 hover:bg-gray-50 transition">
          <div className="text-sm font-medium">Daftar Hazard</div>
          <div className="text-xs text-gray-500">Lihat & tutup tiket</div>
        </Link>
      </div>

      {/* Dashboard KPI dibungkus Suspense (perbaiki error CSR bailout) */}
      <div className="bg-white border rounded-xl p-4">
        <h2 className="text-lg font-semibold mb-3">Dashboard KPI</h2>
        <Suspense fallback={<div className="text-sm text-gray-500">Memuat dashboard…</div>}>
          <DashboardClient />
        </Suspense>
      </div>

      <div className="text-xs text-gray-500">
        KPI dihitung di browser menggunakan akun login aktif. Pastikan koleksi
        <code> hazard_reports </code> dan <code> inspections </code> memiliki field
        <code> createdAt </code>.
      </div>
    </div>
  );
