...
const CATEGORIES = [
  ...
  "Peralatan – Mesin/Guarding",
  "Lingkungan – Kebisingan/Penerangan",
  "Lainnya",
];
...
export default function NewHazardPage() {
  ...
  async function submit() {
    ...
    if (!description.trim()) return setErr("Deskripsi bahaya wajib diisi.");
    ...
    try {
      ...
      // Tanggal kejadian (opsional)—simpan juga sebagai Date ISO agar bisa difilter
      const eventDate = dateStr ? new Date(dateStr) : new Date();

      await addDoc(collection(db, "hazard_reports"), {
        type: "Hazard",                  // <-- pembeda Hazard vs Near Miss
        title: title.trim(),
      ...
      console.error(e);
      setErr(e?.message || "Gagal menyimpan hazard.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-5 max-w-2xl">
      ...
      <div className="bg-white border rounded-xl p-4 space-y-3">
        <div>
          <label className="text-sm text-gray-600">Judul Laporan</label>
          <input
            className="mt-1 border rounded px-3 py-2 w-full text-sm"
            placeholder="Contoh: Lantai licin di area winding"
            value={title}
        ...
        <div>
          <label className="text-sm text-gray-600">Deskripsi Bahaya</label>
          <textarea
            className="mt-1 border rounded px-3 py-2 w-full text-sm min-h-24"
            placeholder="Jelaskan kondisi/kejadian, potensi akibat, pihak terpapar, dsb."
            value={description}
        ...
        <div>
          <label className="text-sm text-gray-600">Tindakan Sementara</label>
          <textarea
        ...
        <div>
          <label className="text-sm text-gray-600">Saran Perbaikan</label>
          <textarea
            className="mt-1 border rounded px-3 py-2 w-full text-sm min-h-20"
      ...
      <div className="text-xs text-gray-500">
        Tips: Gunakan kategori yang paling relevan. Status <b>Open</b> untuk
        tiket yang masih berjalan, <b>Closed</b> jika sudah tuntas.
      </div>
    </div>
  );
}